...
document.oncontextmenu = (e) => {
    ...
    if (e.target.closest('.nav-card') || e.target.closest('.group-header') || e.target.closest('.settings-panel')) return;
...
async function initConfig() {
    ...
    // 应用保存的搜索引擎设置
    currentSearchEngine = appConfig.searchEngine || 'google';
    const iconImg = document.querySelector('#search-engine-icon img');
    const engineConfig = getSearchEngineConfig();
    if (iconImg && engineConfig) {
        iconImg.src = engineConfig.icon;
        iconImg.alt = engineConfig.name;
    }
}
